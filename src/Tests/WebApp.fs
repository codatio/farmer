...
let getSlots (site: WebAppConfig) =
    site
    |> getResources
    ...
    testList
        ...
        [
            ...
            test "WebApp supports adding slots when appName length does not exceed 40 characters" {
                ...
                let site: WebAppConfig =
                    webApp {
                        name "appname-that-is--exactly--40--characters"
                        add_slot slot
                        zip_deploy "test.zip"
                    }
            ...
            test "WebApp does not support adding slots when appName is too long" {
                ...
                let site: WebAppConfig =
                    webApp {
                        name "app-name-that-is-longer-than-40-characters"
                        add_slot slot
                        zip_deploy "test.zip"
                    }
                ...
                let exceptionMessage =
                    Expect.throwsC (fun () -> getSlots site |> ignore) toExceptionMessage
            ...
            test "Supports slot settings" {
                let webApp =
                    webApp {
                        ...
                        slot_settings
                            [
                                "sticky_config", "sticky_config_value"
                                "another_sticky_config", "another_sticky_config_value"
                            ]
                    }
                ...
                let appSettingNames =
                    jobj.SelectTokens
                        $"$..resources[?(@name=='{webApp.Name.ResourceName.Value}/slotconfignames')].properties.appSettingNames[*]"
                    |> Seq.map (fun x -> x.ToString())

                let dependencies =
                    jobj.SelectTokens
                        $"$..resources[?(@name=='{webApp.Name.ResourceName.Value}/slotconfignames')].dependsOn[*]"
                    |> Seq.map (fun x -> x.ToString())
                ...
                Expect.containsAll
                    scn.SlotSettingNames
                    [ "sticky_config"; "another_sticky_config" ]
                    "Slot config names should be set"
                ...
                Expect.containsAll
                    appSettingNames
                    [ "sticky_config"; "another_sticky_config" ]
                    "Slot config names should be present in template"

                Expect.containsAll
                    dependencies
                    [ $"[resourceId('Microsoft.Web/sites', '{webApp.Name.ResourceName.Value}')]" ]
                    "Slot config names resource should depend on web site"
            ...
            test "Supports slot setting" {
                let webApp =
                    webApp {
                        name "test"
                        slot_setting "sticky_config" "sticky_config_value"
                    }
                ...
                let expectedSettings = Map [ "sticky_config", LiteralSetting "sticky_config_value" ]
            ...
            test "Can integrate with private endpoint and add private dns zone group" {
                ...
                Expect.equal
                    (jobj.SelectToken("resources[?(@.name=='webapp-ep')].type").ToString())
                    "Microsoft.Network/privateEndpoints"
                    "privateEndpoint type is wrong"

                Expect.equal
                    (jobj.SelectToken("resources[?(@.name=='webapp-ep')].dependsOn[0]").ToString())
                    "[resourceId('Microsoft.Web/sites', 'farmerWebApp')]"
                    "privateEndpoint for web app should depend on web app"

                Expect.equal
                    (jobj
                        .SelectToken(
                            "resources[?(@.name=='webapp-ep')].properties.privateLinkServiceConnections[0].name"
                        )
                        .ToString())
                    "webapp-ep"
                    "privateEndpoint service connection name is wrong"

                Expect.equal
                    (jobj
                        .SelectToken(
                            "resources[?(@.name=='webapp-ep')].properties.privateLinkServiceConnections[0].properties.groupIds[0]"
                        )
                        .ToString())
                    "sites"
                    "groupIds for privateEndpoint linked to web app should be 'sites'"

                Expect.equal
                    (jobj
                        .SelectToken(
                            "resources[?(@.name=='webapp-ep')].properties.privateLinkServiceConnections[0].properties.privateLinkServiceId"
                        )
                        .ToString())
                    "[resourceId('Microsoft.Web/sites', 'farmerWebApp')]"
                    "privateEndpoint link service id should be the web app."

                Expect.equal
                    (jobj
                        .SelectToken("resources[?(@.name=='webapp-ep')].properties.subnet.id")
                        .ToString())
                    "[resourceId('Microsoft.Network/virtualNetworks/subnets', 'mySubnet')]"
                    "privateEndpoint should be linked with correct subnet"

                Expect.equal
                    (jobj.SelectToken("resources[?(@.name=='webapp-ep/dnszone')].type").ToString())
                    "Microsoft.Network/privateEndpoints/privateDnsZoneGroups"
                    "privateDnsZoneGroups type is wrong"

                Expect.equal
                    (jobj
                        .SelectToken("resources[?(@.name=='webapp-ep/dnszone')].dependsOn[0]")
                        .ToString())
                    "[resourceId('Microsoft.Network/privateEndpoints', 'webapp-ep')]"
                    "privateDnsZoneGroups should depend on private endpoint"

                Expect.equal
                    (jobj
                        .SelectToken(
                            "resources[?(@.name=='webapp-ep/dnszone')].properties.privateDnsZoneConfigs[0].name"
                        )
                        .ToString())
                    "dnszone"
                    "privateDnsZoneGroup name is wrong"

                Expect.equal
                    (jobj
                        .SelectToken(
                            "resources[?(@.name=='webapp-ep/dnszone')].properties.privateDnsZoneConfigs[0].properties.privateDnsZoneId"
                        )
                        .ToString())
                    "[resourceId('Microsoft.Network/privateDnsZones', 'myPrivateDnsZone')]"
                    "privateDnsZoneGroup dns zone id is wrong"
            }
            test "WebApp supports blocking slot-swap" {
                ...
                let app =
                    webApp {
                        name "webapp"
                        add_slot slot
                        zip_deploy_slot "deployment" "some/path"
                    }
